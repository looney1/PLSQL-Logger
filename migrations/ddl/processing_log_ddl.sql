/* Formatted on 6/2/2015 11:43:40 AM (QP5 v5.269.14213.34769) */
BEGIN
   DBMS_OUTPUT.enable (1000000);
   DBMS_OUTPUT.put_line ('Begin processing_log_ddl.sql Script.');
END;
/
...
BEGIN
   EXECUTE IMMEDIATE 'DROP PACKAGE LOGGER_PKG';

   DBMS_OUTPUT.put_line ('Package LOGGER_PKG Dropped.');
EXCEPTION
...
DECLARE
   ...
   PRAGMA EXCEPTION_INIT (e_package_does_not_exist, -4043);
BEGIN
   EXECUTE IMMEDIATE 'DROP PACKAGE BODY LOGGER_PKG';

   DBMS_OUTPUT.put_line ('Package Body LOGGER_PKG Dropped.');
EXCEPTION
   WHEN e_package_does_not_exist THEN
      NULL;
   WHEN OTHERS THEN
...
BEGIN
   EXECUTE IMMEDIATE 'DROP TABLE processing_log CASCADE CONSTRAINTS';

   DBMS_OUTPUT.put_line ('Table processing_log Dropped.');
EXCEPTION
